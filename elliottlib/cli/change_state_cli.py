...
"""
    ...
    try:
        ...
        if e.errata_state == state:
            ...
            return
        elif e.errata_state == 'SHIPPED_LIVE':
            raise ElliottFatalError("Error: Could not change 'SHIPPED_LIVE' advisory {advs}, group.yml is probably pointing at old one".format(advs=advisory))
        else:
            if noop:
