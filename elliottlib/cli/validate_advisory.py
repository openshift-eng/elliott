...
@cli.command("validate-advisory", short_help="Validate RHSA with Product Security checks")
@click.argument('advisory', type=int)
@pass_runtime
def validate_advisory_cli(runtime, advisory):
    """
    Validates an RHSA by returning the 'alerts' generated by Product Security's central SFM2 server.

    Usage:
\b
    $ elliott validate-advisory ID
"""
    ...
    if resp.status_code != 200:
        print(f"HTTP {resp.status_code}: Could not validate advisory {advisory} (is it an RHSA? without embargo?)")
        return
    alerts = resp.json()
